      ...
      >
        <span
          className={`text-base sm:text-lg font-black transition-colors ${
            isOpen
              ? "text-[#D4A017]"
              : "text-[#1D1D4B] group-hover:text-[#D4A017]"
          }`}
        >
          {question}
        </span>
        ...
        <span
          className={`shrink-0 ml-3 transition-transform duration-300 ${
            isOpen ? "rotate-180 text-[#D4A017]" : "text-slate-300"
          }`}
        >
          <svg
...
const FAQs = ({ setActiveTab, setContactSubject }: FAQProps) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 pb-12 animate-in fade-in duration-700">
      {/* Header */}
      ...
      {/* FAQ List */}
      <section className="bg-white rounded-[3rem] p-6 md:p-12 shadow-sm border border-slate-50">
      ...
      {/* CTA */}
      <div className="bg-[#1D1D4B] p-10 rounded-[3rem] text-center text-white space-y-6 relative overflow-hidden">
